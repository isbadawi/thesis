...
preservation. As refactoring writers specify tree-level changes, minimalist
...
\begin{enumerate}
  ...
  \item Otherwise look it up in the original mapping using its position
    information.
\end{enumerate}
...
associated comments. Our heuristic is simple. For each line overlapping a
...
indentation. Given a statement, we can extract its indentation level by
inspecting the portion of the token stream lying between the statement's
fragment and the preceding newline. For a multiline construct like a function,
we extract the indentation of each line and take the common prefix. When
inserting a node, we look for a predecessor or successor node and copy its
indentation. If there are no siblings, we grab the indentation of the parent
node and then try to determine how much to add on. If there are two parent
nodes, we take the difference of their indentation levels. Otherwise, we have
nothing to go on -- a more sophisticated approach might inspect different files
in the project, but each token stream is tied to a single file -- so we fall
back to a default of four spaces (which could be configurable).
