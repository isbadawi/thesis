...
"dominant" type, say \code{foo}. Then, a function defined in any directory
named \code{@foo/} on \matlab's path will have priority over other
...
% TODO(isbadawi): Maybe describe lookup semantics in more detail.
...
the evaluation of arbitrary strings as code via calls to the \code{eval}
...
to these in \chapref{chap:DynamicFeatureElimination}.
...
% TODO(isbadawi): Write about these two things:
% Using handle propagation analysis to avoid instrumenting arrays
% Checking each variable only once at runtime instead of on each use...
