...
these in \chapref{chap:DynamicFeatureSurvey}.
...
\end{tabular}
\caption{Instrumentable expressions in each benchmark. The number in each cell represents the number of expressions instrumented for a given benchmark by a given version of our approach, with the number in parentheses denoting how many of those appear inside loops.}
...
these are in the numerical category, and can be optimized. The functional
...
\begin{itemize}
  ...
  \item $E$ is a use of an identifier $id$. In \matlab, the parentheses can be
    ...
    we had (rule (3)). If it's a call to a function with has an entry in $info$,
...
\subsubsection{Application of handle propagation analysis}
...
the function appears in $A$. If it doesn't, then we can safely avoid instrumenting
...
As an aside, in \secref{sec:CallgraphBuiltins}, we discussed the complications
...
\begin{itemize}
  ...
  \item Our runtime uses the \code{fprintf} functions to log call and enter
    ...
    which behave similarly to their lowercase counterparts, except that the
  ...
  \item Function calls in \matlab are quite expensive. The
    ...
    to do the writing -- inlining this call leads to a significant performance
    boost.
