...
\subsection{\code{eval} and variants}
...
We start by finding all calls to \code{eval} and variants by writing
\code{//ParameterizedExpr[is_call('eval')]}. (Here \code{is_call} is a
...
\subsubsection{Restricted calls to \code{eval}}
...
immediately passed to \code{fprintf}. One legitimate use involves capturing and
parsing the output of the \matlab built-in \code{dbtype} function, which only
displays its output instead of returning it.

\subsubsection{\code{feval}}
...
% TODO(isbadawi): Look at uses? Seems like it's never a good idea :\

\subsubsection{\code{assignin}}
...
\subsubsection{\code{clear} and \code{clearvars}}
...
to a variables called \code{clear} occurring inside scripts. The relevant query
is given in \lstref{Lst:QueryFevalString}. This yields 950 occurrences across
...
comparisons against \code{nargin}, a special \matlab variable that indicates the
number of input parameters passed in, but it's not clear that this would be
worthwhile, or easier to deal with in any significant way.
...
variable. These function turns out not to be as popular, with only 4 calls to
...
% TODO(isbadawi): patterns? Doesn't seem to be any obvious ones.
